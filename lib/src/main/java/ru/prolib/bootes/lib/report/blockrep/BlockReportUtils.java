...
public class BlockReportUtils {
	private static final BlockReportUtils instance = new BlockReportUtils();
	
	public static BlockReportUtils getInstance() {
		return instance;
	}
	
	public CDecimal getAveragePrice(IBlockReport report) {
		CDecimal px = null;
		int pn = 0;
	...
	public Integer getAverageIndex(IBlockReport report, ITimeIndexMapper tim) {
		Integer ix = null;
		int in = 0;
		for ( IBlock block : report.getBlocks() ) {
	 ...
	 */
	public Integer getAverageIndex(IBlockReport report, ITimeIndexMapper tim, int null_time_index) {
		Integer ix = null;
		int in = 0, index;
		for ( IBlock block : report.getBlocks() ) {
			Instant btx = block.getTime();
