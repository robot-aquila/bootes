...
public class BOOTESInitSessionDataTest {
	...
	@Test
	public void testEnter_NotStarted() {
		expect(stateMock.getSessionDataHandler()).andStubReturn(dhMock);
		expect(stateMock.getStateListener()).andStubReturn(rslMock);
		expect(dhMock.startSession()).andReturn(false);
		control.replay();
